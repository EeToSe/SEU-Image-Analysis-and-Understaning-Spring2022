...
from myfunc.iteration import mr_iteration

# file location 
d = Path(__file__).resolve().parents[1]
print(d)
filename = os.path.join(d, r'..\data\ass1\ass1-2\rice.png')
print(filename)

# Top-hat transform
I0 = cv.imread(os.path.join(d, r'..\data\ass1\ass1-2\rice.png'), cv.IMREAD_GRAYSCALE)
I0_th = I0.copy()
I0_th[I0_th <= 150] = 0
I0_th[I0_th > 150] = 255

# compute I_top-hat
se1 =  np.loadtxt(os.path.join(d, r'..\data\ass1\ass1-2\se_1.txt'), delimiter=",").astype(np.uint8)
...
se2 = np.loadtxt(os.path.join(d, r'..\data\ass1\ass1-2\se_2.txt'), delimiter=",").astype(np.uint8)
...
Im = Im - 45
B3 = np.loadtxt(os.path.join(d, r'..\data\ass1\ass1-2\se_3.txt'), delimiter=",").astype(np.uint8)
...
# cv.imwrite("./I0-Ir_infi.png", I0-Ir_infi)